...
- `index.html` — the single-page marketing site. All sections (About, Simulators, Gallery, Pricing, Membership,
...
- `css/style.css` — one shared stylesheet for both pages, organized in commented sections (Header, Hero, About,
  Features, Gallery, Pricing, Membership, Academy, Shop, Events, Terms, Location, Booking form, Footer, Auth
...
- `js/script.js` — `index.html`-only behavior: mobile nav toggle, and the booking form submit handler, which
  currently only logs to console and shows a client-side confirmation message (no backend/API call is wired up
  — see the `TODO` comment there before assuming bookings are actually captured anywhere).
